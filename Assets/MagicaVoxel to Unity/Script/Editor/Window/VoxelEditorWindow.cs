	...
	public partial class VoxelEditorWindow : MoenenEditorWindow {
		...
		private void HeaderGUI () {
			...
			LayoutH(() => {
				...
				// Close Button
				if (ColorfulButton(GUIRect(showSaveButton ? 24 : 36, 24), "×", new Color(1, 0.6f, 0.6f, 1), new GUIStyle(showSaveButton ? EditorStyles.miniButtonMid : EditorStyles.miniButtonLeft) { fontSize = 14 })) {
		...
		private void PickGUI () {
			...
			// Switch Buttons
			{
				...
				rect.width = IsSkeletaling ? BUTTON_WIDTH + 20 : BUTTON_WIDTH;
				rect.x = position.width - rect.width;
				...
				rect.width = IsGeneratingMap ? BUTTON_WIDTH + 20 : BUTTON_WIDTH;
				rect.x = position.width - rect.width;
				...
				rect.width = IsGeneratingCharacter ? BUTTON_WIDTH + 20 : BUTTON_WIDTH;
				rect.x = position.width - rect.width;
				...
				rect.width = IsCombining ? BUTTON_WIDTH + 20 : BUTTON_WIDTH;
				rect.x = position.width - rect.width;
		...
		private void ToolbarGUI () {
			...
			LayoutH(() => {
				...
				// Show Root Button
				if (GUI.Button(tempRect = GUIRect(95, 18), "Show in Hierarchy", EditorStyles.toolbarButton)) {
